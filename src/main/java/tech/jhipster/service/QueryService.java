...
public abstract class QueryService<ENTITY> {
    ...
    protected <X> Specification<ENTITY> buildReferringEntitySpecification(Filter<X> filter, SingularAttribute<? super ENTITY, X> reference) {
        return buildSpecification(filter, root -> root.get(reference));
    }

    protected <X> Specification<ENTITY> buildReferringEntitySpecification(Filter<X> filter, SingularAttribute<? super ENTITY, ? extends Object> reference, SingularAttribute<? super Object, X> idField) {
        return buildSpecification(filter, root -> root.get(reference).get(idField));
    }

    protected <X, OTHER> Specification<ENTITY> buildReferringEntitySpecification(Filter<X> filter, SetAttribute<? super ENTITY, OTHER> reference, SingularAttribute<? super OTHER, X> idField) {
        return buildSpecification(filter, root -> {
            SetJoin<? super ENTITY, OTHER> join = root.join(reference);
            return join.get(idField);
        });
    }
